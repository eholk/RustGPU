...
void jacobi_init() {
   ...
   // Find the platforms
    ...
    // Create a program
    size_t prog_size = jacobi_cl_len;
    const char *prog_src = &jacobi_cl[0];
...
void jacobi(uint64_t N, double *A, double *B) {
    cl_int status;

    int byte_size = N * N * sizeof(double);

    cl_kernel jacobi = clCreateKernel(g_program,
                                      "jacobi",
                                      &status);
    check_status(status);
    ...
    cl_mem clA = clCreateBuffer(g_context,
                                CL_MEM_READ_ONLY,
                                byte_size,
                                NULL, // host pointer...
                                &status);
    ...
    cl_mem clB = clCreateBuffer(g_context,
                                CL_MEM_READ_ONLY,
                                byte_size,
                                NULL, // host pointer...
                                &status);
