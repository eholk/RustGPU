...
fn main() unsafe {
  ...
  /*  let bytes = str::to_bytes(ker);
    ...
    let binary = io::read_whole_file_str(&path::Path(@"kernel.ptx")).get();
    let prog = do str::as_c_str(binary) |kernel_ptr| {
        clCreateProgramWithBinary(ctx, nd, ptr::addr_of(&device), ptr::addr_of(&(binary.len() + 1)) as *libc::size_t, ptr::addr_of(&kernel_ptr) as **libc::c_uchar, ptr::null(), ptr::null())
    };
    ...
    // Get the binary output of the program.
    
    /* let kernel_length = 0;
    
    let program = clGetProgramInfo(prog, CL_PROGRAM_BINARY_SIZES, sys::size_of::<int>() as libc::size_t, ptr::addr_of(&kernel_length) as *libc::c_void, ptr::null());
    let binary = vec::from_elem(kernel_length, 0u8);
    do vec::as_imm_buf(binary) |binary, _len| {
        clGetProgramInfo(prog, CL_PROGRAM_BINARIES, kernel_length as libc::size_t, ptr::addr_of(&binary) as *libc::c_void, ptr::null()); 
    }
    io::println(kernel_length.to_str());
    let binary = str::from_bytes(binary);
    io::println(binary);
    */
